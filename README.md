...
- **V3.5**（1.3.5）：**三主题逐字歌词（卡拉OK逐词扫过，PR #2 by 电狗）**——AM/FB2K 逐词 span 渲染、粒子舞台原生逐字接管；长歌词行**折行显示不再缩字**，支持「每行词数」限制；歌词外观自定义（字号 70–160% / 行距 / 每行词数，持久化）；沉浸/迷你歌词滚动条仅滚动时出现；lxlyric 行格式（`[起始ms,时长ms]`）支持；内嵌歌词读取修复
