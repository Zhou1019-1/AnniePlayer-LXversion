...
当前版本：**V4.1.0**（AM 主打界面动效体系：启动序列/入场错峰/切歌过渡/微交互，三档可调）
...
- **曲目列表操作**：单击播放；♡ 收藏；拖放文件到列表上/下半插入对应位置
...
- **可视化面板（📊）**：三个页签：整曲波形 / Spek 风格频谱图 / 无损检测（四方法加权融合：频谱截止/编码帧痕迹/位深量化/上转换，结论 + 得分 + 逐项理由）
...
- **常规**：粒子总开关 / 封面氛围背景 / 托盘驻留 / 全局快捷键 / 界面主题 / 强调色色板（7 色，三主题同步）/ AM 频谱条开关 / 氛围模式入口 / FB2K 暗色 / 配色方案四卡 / 界面动效三档（完整/精简/关闭）
...
- **曲库工具**：响度补算 / 无损鉴别（整库/文件夹/单曲三种范围，四方法加权融合，可导出 CSV/HTML 报告，不在列表打标记）/ 重复歌曲清理（移回收站可恢复）/ 导出诊断包（zip）/ 复制诊断摘要（轻量报障直接粘贴）/ 听歌报告
...
- **AutoEq 耳机校正**：设置 → 播放 → 参量均衡器下方：输入耳机型号即可套用 AutoEq 实测校正曲线（内置 189 款热门型号精选子集，数据来自 oratory1990 / crinacle / Rtings 等测量来源）；低架/高架滤波以峰值滤波近似，AutoEq 前级增益由自动前级补偿接管，无需手动降音量
